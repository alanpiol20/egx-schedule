...
function entriesToPresences(entries: DayEntry[]): DayPresence[] {
  ...
  for (const e of entries) {
    ...
    for (const line of e.amScheduled) {
      const p = parseDriverLine(line);
      if (p) {
        amSet.add(p.id);
        if (!idToName.has(p.id)) idToName.set(p.id, p.name);
      }
    }
    for (const line of e.pmScheduled) {
      const p = parseDriverLine(line);
      if (p) {
        pmSet.add(p.id);
        if (!idToName.has(p.id)) idToName.set(p.id, p.name);
...
export default function App() {
  ...
  const [dayEntries, setDayEntries] = useState<DayEntry[]>(() => {
    try {
      const raw = localStorage.getItem("egx_day_entries_v2");
  ...
  // Carrega últimos 14 dias (apenas logado)
  useEffect(() => {
    if (!userEmail) return;
  ...
  async function saveDay() {
    ...
    // local
    const next = [...dayEntries.filter((d) => d.date !== entry.date), entry].sort(
      (a, b) => a.date.localeCompare(b.date)
    );
    setDayEntries(next);
    try {
      localStorage.setItem("egx_day_entries_v2", JSON.stringify(next));
    } catch {}
    ...
    try {
      ...
      alert(
        online
          ? "Dia salvo!"
          : "Dia salvo (offline). Sincroniza quando voltar a internet."
      );
    } catch (e) {
      console.error(e);
  ...
  function clearAll() {
    if (!confirm("Apagar TODOS os dias localmente? (não remove do Firestore)"))
      return;
    setDayEntries([]);
    setSelectedDates({});
    try {
      localStorage.removeItem("egx_day_entries_v2");
    } catch {}
  }
  ...
  async function deleteSingleDate(date: string) {
    ...
    try {
      ...
      console.error(e);
      alert("Falha ao apagar na nuvem.");
      return;
    }
  ...
  async function deleteSelectedDates() {
    const dates = Object.entries(selectedDates)
      .filter(([, v]) => v)
      .map(([k]) => k);
    if (dates.length === 0) {
    ...
    const proceed = confirm(
      `Apagar ${dates.length} dia(s) selecionado(s) localmente e no Firestore?`
    );
    if (!proceed) return;
    try {
      await deleteDays(dates);
    } catch (e) {
      console.error(e);
      alert("Falha ao apagar na nuvem.");
      return;
  ...
  const rows = useMemo(
    () => aggregateDays(presences, thisWeekStart),
    [presences, thisWeekStart]
  );
      ...
      <div className="app">
        ...
        <section className="card" style={{ maxWidth: 420 }}>
          <form onSubmit={doLogin} style={{ display: "grid", gap: 8 }}>
            ...
            {!online && (
              <div className="subtitle">
                Você está offline. O login requer internet.
              </div>
            )}
          </form>
        </section>
    ...
    <div className="app">
      ...
      {!online && (
        <section className="card" style={{ borderColor: "#f59e0b" }}>
          Você está <b>offline</b>. O Firestore funciona em modo offline e
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button
              ...
              onClick={() => {
                ...
                a.href = url;
                a.download = `egx-days-summary-${Date.now()}.csv`;
                a.click();
            ...
            <button className="btn" onClick={clearAll}>
              Limpar TODOS os dias (local)
            </button>
            <button className="btn primary" onClick={deleteSelectedDates}>
              Apagar selecionados (local + nuvem)
            </button>
          </div>
        </section>
      </section>
      ...
      <section className="card" style={{ marginTop: 12 }}>
        ...
        <div className="grid" style={{ marginTop: 10 }}>
          <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <textarea
              className="textarea"
              value={amScheduleText}
              onChange={(e) => setAmScheduleText(e.target.value)}
              placeholder={`1011982 Karanjot (EGX Logistics)\n1012219 Nicolas (EGX Logistics)`}
            />
          </label>
          <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <textarea
              className="textarea"
              value={amOffText}
              onChange={(e) => setAmOffText(e.target.value)}
            />
          </label>
          <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <textarea
              className="textarea"
              value={pmScheduleText}
              onChange={(e) => setPmScheduleText(e.target.value)}
            />
          </label>
          <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <textarea
              className="textarea"
              value={pmOffText}
              onChange={(e) => setPmOffText(e.target.value)}
            />
          </label>
        </div>
                ...
                return (
                  <div key={date} className="date-chip">
                    ...
                    <button
                      ...
                      onClick={() => deleteSingleDate(date)}
                      title={`Apagar ${date} (local + nuvem)`}
                    >
                      🗑️
                    </button>
                  </div>
      ...
      <section className="card" style={{ marginTop: 12 }}>
        ...
        >
          <div style={{ fontWeight: 600 }}>
            Resumo por driver (últimos 14 dias)
          </div>
        </div>

        {rows.length === 0 ? (
          <div className="subtitle" style={{ padding: 8 }}>
            Sem dados ainda. Salve um dia (AM/PM).
          </div>
        ) : (
          <>
            {/* DESKTOP/TABLET: tabela clássica */}
                      ...
                      <td>
                        {r.suggest ? (
                          <span className="badge-warn">
                            Sugerir Day Off ({r.reasons.join("; ")})
                          </span>
                        ) : (
            ...
            {/* MOBILE: cards bonitos e responsivos */}
            <div className="mobile-only summary-cards">
              {rows.map((r) => (
                <div key={r.driverId} className="summary-card">
                  <div className="summary-card-header">
                    <div className="summary-driver-name">{r.driverName}</div>
                    {r.suggest ? (
                      <span className="badge-warn">Sugerir Day Off</span>
                    ) : (
                      <span className="badge-ok">OK</span>
                    )}
                  </div>
                  ...
                  <div className="summary-stats">
                    <div className="summary-pill">
                      <span className="label">LW</span>
                      <span className="value">{r.daysLastWeek}</span>
                    </div>
                    <div className="summary-pill">
                      <span className="label">TW</span>
                      <span className="value">{r.daysThisWeek}</span>
                    </div>
                    <div className="summary-pill">
